...
def offline_alg(jobs, Tt, add_interval_cuts= True, time_limit = 60):
    ...
    x = {}
    y = {}
    #--- Variables ( only inside windows ) ---
    for j, job in enumerate(jobs):
        y[j] = model.NewBoolVar(f"y_{j}")
        for t in range(job["r"], job["d"] + 1):
            x[j, t] = model.NewBoolVar(f"x_{j}_{t}") #j runs in slot t
    
    #--- Capacity : at most one job per slot ---
    for t in range(1, Tt +1):
        model.Add(sum(x[j, t] for j, job in enumerate(jobs) if (j, t) in x) <= 1)
    #--- Completion link per job ---
    for j, job in enumerate(jobs):
        model.Add(sum(x[j, t] for t in range(job["r"], job["d"] + 1)) == job["p"] * y[j])

    #--- Optional : demand - bound cuts over all intervals [a , b ] ---
    ...
    #Extract schedule : pick job with x [j , t ]=1 in each slot164
    schedule ={}
    completed, failed = [], []
